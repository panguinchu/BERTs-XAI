...
# path to cola data
cola_data_path = "../"
cola_public_raw_subpath = "SST-2/train.tsv"
cola_data = pd.read_csv(cola_data_path + cola_public_raw_subpath, delimiter = '\t', header = None, names = ['sentence', 'label'])

# Extrat the parts we need, (1) sentences (2) the label
 
cola_sentences = cola_data.sentence.values[1:]
cola_labels = [int(i) for i in cola_data.label.values[1:]]
print(type(cola_labels[0]))
...
if test:
    ...
    cola_public_raw_testing_subpath = "SST-2/dev.tsv"
    cola_test_data = pd.read_csv(cola_data_path + cola_public_raw_testing_subpath, delimiter = '\t', header = None, names = ['sentence', 'label'])
    test_sentences = cola_test_data.sentence.values[1:]
