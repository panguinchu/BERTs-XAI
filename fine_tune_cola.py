...
# path to cola data
cola_data_path = "../"
cola_public_raw_subpath = "cola_public/raw/in_domain_train.tsv"
cola_data = pd.read_csv(cola_data_path + cola_public_raw_subpath, delimiter = '\t', header = None, names = ['sentence_source', 'label', 'label_notes', 'sentence'])

# Extrat the parts we need, (1) sentences (2) the label
cola_sentences = cola_data.sentence.values
cola_labels = cola_data.label.values
print(cola_sentences[0])
print("type of cola_labels[0]",type(cola_labels[0]))
if train:
    # Tokenizing
    ...
    for sentence in cola_sentences:
    ...
    train_x, val_x, train_y, val_y = train_test_split(sent_ids, cola_labels, random_state = 1000, test_size = 0.1)
    train_m, val_m, _, _ = train_test_split(attn_masks, cola_labels, random_state = 1000, test_size = 0.1)
    ...
    # stdout
    original = sys.stdout
    f = open('cola_fine_tune_{}_{}.txt'.format(bert_type, 'train'), 'w')
    sys.stdout = f
    ...
    plt.savefig("{}_fine_tune_cola_loss.png".format(bert_type))

    # Save the trained model
    save_dir = './cola_models_saved/{}/'.format(bert_type)
...
model_save_dir = 'cola_models_saved/{}'.format(bert_type)
...
if test:
    ...
    if fine_tune:
        ...
        MODELS = [(BertForSequenceClassification, BertTokenizer, PretrainedBert)] 
        original = sys.stdout
        f = open('cola_fine_tune_{}_{}.txt'.format(bert_type, 'test'), 'w')
        sys.stdout = f
    else:
        original = sys.stdout
        f = open('cola_fine_tune_{}_{}_orig.txt'.format(bert_type, 'test'), 'w')
        sys.stdout = f
    ########### Testing ##############
    for model_class, tokenizer_class, pretrained_weights in MODELS:
        # Load pretrained model/tokenizer
    ...
    cola_public_raw_testing_subpath = "cola_public/raw/out_of_domain_dev.tsv"
    cola_test_data = pd.read_csv(cola_data_path + cola_public_raw_testing_subpath, delimiter = '\t', header = None, names = ['sentence_source', 'label', 'label_notes', 'sentence'])
    test_sentences = cola_test_data.sentence.values
    test_labels = cola_test_data.label.values
    ...
    # Convert the data into torch.tensor
    test_x, test_y = torch.tensor(test_sent_ids), torch.tensor(test_labels)
    ...
    correct_sentences_pos = [[test_sentences[index], index] for index in correct_list if test_labels[index]]
    correct_sentences_neg = [[test_sentences[index], index]  for index in correct_list if not test_labels[index]]
    wrong_sentences_pos = [[test_sentences[index], index]  for index in wrong_list if test_labels[index]]
    wrong_sentences_neg = [[test_sentences[index], index] for index in wrong_list if not test_labels[index]]
    ...
    for t in correct_or_wrong:
        ...
        if fine_tune:
            output_dir = './cola_visual/{}/{}/'.format(bert_type, t)
        else:
            output_dir = './cola_visual_orig/{}/{}/'.format(bert_type, t)
        if fine_tune:
            output_cca_dir = './cola_visual/{}/{}/'.format(bert_type, "cca")
        else:
            output_cca_dir = './cola_visual_orig/{}/{}/'.format(bert_type, "cca")
        ...
        for j in range(min(test_min,len(list_of_sentences))):
            input_ids = torch.tensor([tokenizer.encode(list_of_sentences[j][0])]).to(device)
            if not cca_only:
                ...
                if fine_tune:
                    with open("{}/tokens_{}_{}_{}_{}.txt".format(output_dir,data_type, bert_type, t,list_of_sentences[j][1]), "wb") as fp:   #Pickling
                        pickle.dump(token_ids, fp)
                else:
                    with open("{}/tokens_{}_{}_{}_{}_orig.txt".format(output_dir,data_type, bert_type, t,list_of_sentences[j][1]), "wb") as fp:   #Pickling
                        pickle.dump(token_ids, fp)
            ...
            # TO VISUALIZE atten
                if fine_tune:
                    with open("{}/probe_attention_weight_output_{}_{}_{}_{}.txt".format(output_dir,data_type, bert_type, t, list_of_sentences[j][1]), "wb") as fp:   #Pickling
                        pickle.dump(attention_vanilla, fp)
                else:
                    with open("{}/probe_attention_weight_output_{}_{}_{}_{}_orig.txt".format(output_dir,data_type, bert_type, t, list_of_sentences[j][1]), "wb") as fp:   #Pickling
                        pickle.dump(attention_vanilla, fp)
    ...
    if plot:
        ...
        for i in range(layer_size):
            for j in range(layer_size):
                a_layer, b_layer = stack_activation[i,:,:].detach().cpu().numpy(), stack_activation[j,:,:].detach().cpu().numpy()
        ...
        with open("{}/matrix_robust_cca_score_{}_{}_{}.txt".format(output_cca_dir, pooling_method, dataset, bert_type), "wb") as fp:   #Pickling
    ...
    if plot:
        ...
        plt.colorbar()
        plt.savefig('{}/vis_robust_cca_score_{}_{}_{}.png'.format(output_cca_dir, pooling_method, dataset, bert_type))
